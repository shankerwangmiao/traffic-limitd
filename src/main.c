/* Copyright xhawk, MIT license */

#include <stdio.h>
#include <systemd/sd-event.h>
#include <s_task.h>
#include <se_libs.h>
#include <errno.h>
#include <string.h>
#include <signal.h>
#include <log.h>
#include <stdlib.h>


static s_event_t  g_event;
static const size_t STACK_SIZE = 256*1024;
static bool       g_closed = false;

static sd_event *g_sd_event;


static void sub_task(__async__, void *arg) {
    size_t n = (size_t)arg;

    printf("begin_sub_%ld\n", n);

    while (!g_closed) {
        s_event_wait(__await__, &g_event);
        printf("task %d wait event OK\n", (int)n);
    }
}


static void main_task(__async__, void *arg) {
    int i;
    s_event_init(&g_event);

    printf("begin_main\n");

    //s_task_create(g_stack0, sizeof(g_stack0), sub_task, (void *)1);
    //s_task_create(g_stack1, sizeof(g_stack1), sub_task, (void *)2);
    se_task_create(g_sd_event, STACK_SIZE, sub_task, (void *)1);
    se_task_create(g_sd_event, STACK_SIZE, sub_task, (void *)2);

    for(i = 0; i < 10; ++i) {
        printf("task_main arg = %p, i = %d\n", arg, i);

        s_task_yield(__await__);
        /* s_task_msleep(__await__, 500); */
        se_task_usleep(__await__, g_sd_event, 500*1000);

        if (i % 3 == 0) {
            printf("task main set event\n");
            s_event_set(&g_event);
        }
    }

    g_closed = true;
    s_event_set(&g_event);
}

static int exit_req_handler(sd_event_source *s, const struct signalfd_siginfo *si, void *userdata){
    sd_event_source_disable_unref(s);
    int rc = sd_event_exit(g_sd_event, 0);
    if(rc < 0){
         log_error("sd_event_exit failed: %s", strerror(-rc));
    }
    return 0;
}

static int install_signals(void){
    sigset_t mask;
    sigaddset(&mask, SIGINT);
    sigaddset(&mask, SIGTERM);
    int rc = 0;
    rc = sigprocmask(SIG_BLOCK, &mask, NULL);
    if(rc < 0){
        return -errno;
    }
    return rc;
}

int main(int argc, char *argv[]) {
    (void)argv;

    if(getenv("SYSTEMD")){
        log_set_systemd(true);
    }

    s_task_init_system();

    log_trace("init_sys");

    int rc = 0;

    rc = install_signals();
    if(rc < 0){
        log_error("install_signals failed: %s", strerror(-rc));
        return -1;
    }

    rc = sd_event_default(&g_sd_event);

    if(rc < 0){
        log_error("get sd event failed: %s", strerror(-rc));
        return -1;
    }

    rc = sd_event_add_signal(g_sd_event, NULL, SIGINT, exit_req_handler, NULL);
    if(rc < 0){
        log_error("add signal failed: %s", strerror(-rc));
        return -1;
    }
    rc = sd_event_add_signal(g_sd_event, NULL, SIGTERM, exit_req_handler, NULL);
    if(rc < 0){
        log_error("add signal failed: %s", strerror(-rc));
        return -1;
    }

    //s_task_create(g_stack_main, sizeof(g_stack_main), main_task, (void *)(size_t)argc);
    se_task_create(g_sd_event, STACK_SIZE, main_task, (void *)(size_t)argc);

    log_trace("main_create");

    while(1){
        s_task_main_loop_once();
        rc = sd_event_run(g_sd_event, (uint64_t) -1);
        if(rc == -ESTALE){
            break;
        }else{
            if(rc < 0){
                log_error("sd_event_run failed: %s", strerror(-rc));
                return -1;
            }
        }
    }
    log_info("all task is over");
    sd_event_unrefp(&g_sd_event);
    return 0;
}
